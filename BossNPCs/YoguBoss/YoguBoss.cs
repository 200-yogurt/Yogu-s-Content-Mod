...
namespace YoguContentMod.NPCs.BossNPCs.YoguBoss
{
    public class YoguBoss : ModNPC
    {
        ...
        {
            DisplayName.SetDefault("King Yogurt");
        }
        ...
        const int SecondaryTimerSlot =21;
        ...
        { 
            ...
            else if(State == AIState.RandomProjectileShooting)
            {
                if(Timer < 20 && npc.life >= npc.lifeMax * 0.4f) // if npc is behind 40% projectile spam, otherwise, every 0.3 secs (20 ticks)
                {
                    Timer++; 
                }
                else // spawn projectile
                {
                    ...
                    Projectile proj = Projectile.NewProjectileDirect(npc.Center, direction, ProjectileID.MeteorShot, 9, 1f, Main.myPlayer); 
                    proj.friendly = false; 
                    proj.hostile = true;
                }
            }    
